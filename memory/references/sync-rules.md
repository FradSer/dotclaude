...
`/memory:consolidate sync` and `/memory:consolidate publish` translate facts between the private harness memory (Tier A) and the repo-local memory (Tier B). Both schemas are stable; the translation is mechanical except for the conflict cases below.
...
4. **Private blocks outbound.** A Tier A fact with `visibility: private` (or unset) is never pushed to Tier B, even if a Tier B copy exists and is older. The user must `/memory:consolidate publish <file>` it first.
