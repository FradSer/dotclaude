...
2. **Recently modified code**: If no arguments, focus on code that has been recently modified or touched in the current session (like .research/claude-plugins-official/code-simplifier/)
...
1. **Identify Target Code**:
   ...
   - If no arguments: Focus on code that has been recently modified or touched in the current session
   - Invoke **@code-simplifier** agent with clear scope context:
     - If arguments: "Please use File/Directory Scope to refactor these files: `$ARGUMENTS`"
     - If no arguments: "Please use Recent Changes Scope to refactor code modified in the current session"
...
- **Identification**: Determine target scope (arguments or current session modifications)
