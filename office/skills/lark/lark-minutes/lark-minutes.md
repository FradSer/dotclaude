...
| 拿这条妙记关联的纪要文档（`note_doc_token` / `verbatim_doc_token` / `shared_doc_tokens`） | `+detail` 取顶层 `note_id` → [`note +detail --note-id`](../lark-note/lark-note.md) |
...
| 用户同时提到"会议/开会"和"妙记" | 先 [lark-vc](../lark-vc/lark-vc.md)（`+search` → `+recording`）获取 `minute_token`，再本 skill |
...
1. 如果是会议的妙记，应优先通过 [lark-vc](../lark-vc/lark-vc.md) 定位会议并获取 `minute_token`。
...
| 我的任务清单 | — | 走 [lark-task](../lark-task/lark-task.md) |
...
4. 新说话人姓名用 [lark-contact](../lark-contact/lark-contact.md) 解析为 `ou_` open_id。
...
`minutes +detail` 顶层直接返回 `note_id`（仅在该妙记关联纪要时存在）。不需要绕回 [lark-vc](../lark-vc/lark-vc.md)，直接：
...
- 搜索历史会议记录、查参会人快照 → [lark-vc](../lark-vc/lark-vc.md)
- 未来日程 / 日历查询 → [lark-calendar](../lark-calendar/lark-calendar.md)
- 已知 `note_id` 直接读纪要详情 → [lark-note](../lark-note/lark-note.md)
- 飞书任务清单（个人 Todo / 共享清单） → [lark-task](../lark-task/lark-task.md)
- 只有自然语言纪要标题、没有 `minute_token` / 妙记 URL / 本地音视频时定位逐字稿 → 文档搜索（[lark-drive](../lark-drive/lark-drive.md) / [lark-doc](../lark-doc/lark-doc.md)）
