...
*   **One-off prototypes** — throwaway code answering a design question (see `/mattpocock:prototype`). It will be deleted, not shipped, so a regression guard adds nothing. If the prototype survives into the real codebase, the Iron Law re-applies.
