---
name: tdd
description: "Test implementation reference for BDD's Automation phase — red-green loop mechanics, seams, mocking, and test quality. Use when the user wants to write or improve tests during BDD implementation, needs guidance on test structure, or asks about test seams, mocking, or anti-patterns."
---

# TDD as BDD Automation

BDD cycles through three phases: **Discovery** (conversations → examples), **Formulation** (Gherkin scenarios), **Automation** (red-green-refactor). This skill is the **Automation** reference — the how of test implementation, once `/mattpocock:bdd` has defined the what.

When invoked during `/mattpocock:implement` or `/mattpocock:bdd`, use this skill as the test-writing authority. The `/mattpocock:bdd` skill governs the scenario design and Iron Law; this skill governs the test code quality.
...
## CRITICAL: BDD Automation is not standalone TDD

This skill is the **Automation** phase of the BDD lifecycle. It is never invoked in isolation — it always runs under `/mattpocock:bdd` or `/mattpocock:implement`:
...
Do not start writing test code until the Gherkin scenario is defined and the seam is agreed. "Test first" means the scenario is written first; the test implementation code follows.
