...
Reachable only when you type the skill name (Claude Code: `disable-model-invocation: true` — the model will not auto-invoke them; another skill cannot reach them either).
...
- **[implement](./implement/SKILL.md)** — Build the work described by a spec or set of tickets, driving `/mattpocock:bdd` at pre-agreed seams and closing out with `/mattpocock:code-review` before committing.
...
- **[wizard](./wizard/SKILL.md)** — Generate an interactive bash wizard that walks a human through a manual procedure (provisioning infrastructure, setting up credentials or CI secrets, walking an unfamiliar dashboard, or a one-off migration) — opening URLs, capturing values, writing `.env` and GitHub Actions secrets. Model-invoked.
