...
5. **Sync local base branch** (only after a successful merge) — fetch the PR's `baseRefName` (`gh pr view "$PR" --repo "$REPO" --json baseRefName -q .baseRefName`). **CRITICAL: check for a linked worktree first.** If `git rev-parse --absolute-git-dir` differs from `git rev-parse --path-format=absolute --git-common-dir`, this session is in a linked worktree (e.g. `/github:resolve-issues` delegated here) — run `git fetch origin <baseRefName>` ONLY and never `git switch`. Switching there either hard-fails (base checked out elsewhere) or silently drags the issue worktree off its head branch onto the base, where a later `ExitWorktree action:"remove"` would delete the base branch. In the main worktree, `git switch <baseRefName>` + `git pull --ff-only`; if it is on the head branch, switch before pulling; if uncommitted changes remain from earlier in the session, stash or report rather than aborting the pull. Do NOT delete branches (local or remote) — that is the user's call, not this skill's. Skip this step entirely if `gh pr merge` failed or the user chose not to merge. See `references/closeout.md` for the guarded snippet.
