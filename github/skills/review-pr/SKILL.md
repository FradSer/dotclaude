...
5. **After merge** — finish head cleanup if needed; `fetch --prune` and fast-forward local `main` and `develop` when present; prune merged locals / stale worktrees. In a linked worktree: delete remote head if stack-safe, fetch only for long-lived branches — do not switch or delete the local issue head (`ExitWorktree` owns that). Skip if merge failed or user chose not to merge.
