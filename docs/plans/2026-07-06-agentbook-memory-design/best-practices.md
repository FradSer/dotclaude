...
- Integration-test the consumer touchpoints (`autoresearch`, `systematic-debugging`, `github:review-pr`) with `commons-bridge` absent entirely — confirm each consumer's core workflow still completes, proving the dependency truly degrades to a no-op rather than a hard failure.
