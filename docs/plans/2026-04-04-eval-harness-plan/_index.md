...
**Goal:** Replace rubric-based scoring in superpowers-evaluator with binary PASS/FAIL checklist evaluation, add intra-plan learning and batch-boundary context management to executing-plans.

**Architecture:** Two subsystems. Subsystem A updates the evaluator agent to read versioned checklist files and produce binary results instead of 1-5 scores. Subsystem B adds recurring failure pattern detection to executing-plans Phase 4 and batch-boundary context management.

**Tech Stack:** Markdown-based Claude Code plugin files (agent definitions, skill definitions, checklists), Python validation script.
...
**Executor notes:**
- Tasks 005, 006, 007 all modify `superpowers/agents/superpowers-evaluator.md` (different sections). Serialize within a batch (not parallel) to avoid merge conflicts — three agents editing different markdown sections of one file is brittle even with worktrees.
- Tasks 011, 012 both modify `superpowers/skills/executing-plans/SKILL.md` (different sections). Same serialization applies.
- Task 012 scope covers ALL three skills (executing-plans, writing-plans, brainstorming).
...
- [Task 012: Remove old rubrics and update cross-references (ALL skills)](./task-012-remove-rubrics-update-refs.md)
- [Task 013: Update plugin.json and validate plugin](./task-013-plugin-json-validation.md)
...
task-001 (setup)
    ...
    |     +--------------------------------------------------------------+
    |     v
    |     task-008 (shared-standards + output responsibility)
    |         |
    |         v
    |     task-009 (file-formats + cost tracking)
    |         |
    |         v
    |     task-010 (phase-3f + feedforward)
    |      /         \
    |     v           v
    |  task-011    task-012 (remove-rubrics, ALL skills)
    |  (phase-4       |
    |  + context)     |
    |     |           |
    |     v           v
    +----> task-013 (plugin.json) <--- 011+012
```

**Analysis:**
- No circular dependencies
- 13 tasks total, linear critical path through 001→002→005→008→009→010→011→013
- Maximum parallelism at Tier 1 (tasks 002, 003, 004) and Tier 2 (tasks 005, 006, 007)
- Tasks 005/006/007 modify the same file -- **serialize, do not parallelize** (markdown section merge is fragile)
- Tasks 011/012 both modify executing-plans/SKILL.md -- same serialization applies
