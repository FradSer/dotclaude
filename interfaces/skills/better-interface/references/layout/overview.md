...
Hit-area sizes and focus behavior are covered by `references/accessibility/overview.md`; visual polish (radius, shadows, animation) by `references/ui/overview.md`; line length and text spacing by `references/typography/overview.md`.
...
Without an established density system, start with `12px` between adjacent bordered or filled controls and `24px` of clearance around borderless text- and icon-only controls. Compact layouts may use less when `references/accessibility/overview.md` hit areas do not overlap and the controls remain visually distinct.
