...
This is a **Codex plugin marketplace** (`frad-dotclaude`) containing 21 plugins across development and productivity categories. Each plugin follows auto-discovery conventions—place components in `commands/`, `agents/`, `skills/` directories and Codex discovers them automatically.
...
```
plugin-name/
├── .Codex-plugin/plugin.json  # Manifest with commands/agents/skills/hooks
...
**Skill registration determines visibility.** A skill registered under `"commands"` in plugin.json becomes a user-invocable slash command (e.g., `/git:commit`). A skill registered under `"skills"` is internal-only, loaded automatically when Codex needs domain knowledge but never shown in `/help`. Example from `refactor/`:
- `"commands": ["./skills/refactor/"]` -- user runs `/refactor:refactor`
- `"skills": ["./skills/best-practices/"]` -- loaded automatically during refactoring

**Hooks** can be inline in `plugin.json`. See `git/.Codex-plugin/plugin.json` for the `PreToolUse` hook pattern (runs a shell script to validate Bash tool calls before execution).
...
**Version sync:** Plugin versions in individual `plugin.json` files are authoritative. Keep `.Codex-plugin/marketplace.json` entries in sync when bumping versions. `README.md` / `README.zh-CN.md` are a third sync target — their plugin listings drift silently (currently still mention removed `next-devtools`/`shadcn`), so run `/utils:update-readme` whenever you add, remove, or rename a plugin.
...
1. `mkdir -p plugin-name/{.Codex-plugin,skills,agents}`
2. Add `plugin.json` with name, description, author, version, keywords, license
3. Add entry to `.Codex-plugin/marketplace.json` with matching version
...
### Key rules
- **Commands** write directives TO Codex, not descriptions FOR users. Use `$ARGUMENTS`, `${CLAUDE_PLUGIN_ROOT}` for dynamic context. Never use bare `Bash` in `allowed-tools`.
