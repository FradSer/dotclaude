...
3. **Resolve design checklist**: Use the spawn-provided path when present. Otherwise pick the highest `docs/retros/checklists/design-v{N}.md` by numeric `N` (not `design-v1.md` when newer versions exist — same resolution rule as code mode).
...
5. **Produce rework items**: for each FAIL -- Item ID, file, location, what failed, corrective action. JUST-01 rework template: see `docs/retros/checklists/design-v1.md` JUST-01 Rework format (two paths: remove the marker after fixing the activation gate, or move to a single-file reject-form retro under `docs/retros/`). The v1 file holds the full JUST-01 Description/Anchor/Evidence/Rework prose; later versions inline only the check method and verdict precedence, so `design-v1.md` is the authoritative source for the rework template.
