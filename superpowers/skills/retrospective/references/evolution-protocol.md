...
The emitter auto-injects `$timestamp` and `$repo_root`. The hook derives `$files` from `handoff-state.md` dual-format extraction — backtick items under `## Modified Files (cumulative)` (legacy form) **or** the first-column paths of the `## File Ownership` table (current `handoff-template.md` form) — then `$cc` as the most recent commit touching that set (`git log -1 --format=%h -- <files>`). This both confirms Phase 5 committed and yields the completion anchor rather than a bare `HEAD` guess. `$plan` is the repo-relative plan path, `$tc` is the completed-task count from either `## Completed Task IDs` bullets (legacy) or `## Completed Tasks` table rows (current), `$bc` the active `sprint-contract-batch-*.md` count (archived `*.vN.md` excluded). First-completion-only dedup uses `lib/jsonl-emit.sh` `dedup_check` with anchored substring `,"plan":"<path>"` on the log tail (same spirit as the deleted `loop.sh` writer).
...
- `task_count` / `batch_count` — enrichment from `handoff-state.md` (`## Completed Task IDs` bullets **or** `## Completed Tasks` table rows) and active `sprint-contract-batch-*.md` count. Default 0 when unparseable.
...
- `completion_modified_files` — repo-relative paths from `handoff-state.md`: backtick items under `## Modified Files (cumulative)` (legacy) **or** first-column paths under `## File Ownership` (current template). Defaults to `[]`. Restricts post-plan-diff scope to plan-related files.
