...
本文档记录 superdev 插件如何与上游 [mattpocock/skills](https://github.com/mattpocock/skills) 保持同步。**每次同步前先读此文档**——它包含上次同步（2026-08-07，v1.2.0 → v1.2.3）踩过的坑和固化的决策。
...
| 本地插件版本 | **0.1.3**（`superdev/.claude-plugin/plugin.json`） |
| 注册 skill 数 | **26** |
...
diff -rq /tmp/skills-new/skills superdev/skills | grep "Only in /tmp/skills-new"

# C. 内容差异：本地 vs 上游（判定哪些是定制、哪些该吸收）
diff -rq /tmp/skills-new/skills superdev/skills | grep "differ"
```
...
逐文件 `diff` 上游新旧版本，对上游改动的正文段落用 Edit 吸收，**保留**本地 frontmatter（description/`disable-model-invocation`）、`/superdev:` 前缀、`CRITICAL:` 块、`AskUserQuestion` 工具化改写。
...
for f in $(find /tmp/skills-new/skills -type f | sed 's|/tmp/skills-new/skills/||'); do
  [ -f "superdev/skills/$f" ] || echo "MISSING: $f"
done
```
...
| 决策 | 规则 |
|---|---|
| **tdd** | 完整镜像到 `skills/engineering/tdd/`，**不注册**到 plugin.json（bdd 是它的替代） |
| **bdd** | 本地替代 tdd，注册；参考 tdd 形态建 `agents/openai.yaml`（`display_name: "BDD"` / `short_description: "Behavior-driven red-green-refactor"`） |
...
- 所有命令用 `/superdev:` 前缀（**禁止裸 `/tdd`、`/setup-matt-pocock-skills` 等**）
...
| 文件 | 改什么 |
|---|---|
| `superdev/.claude-plugin/plugin.json` | version、description（v1.2.x）、`skills` 数组增删 |
| `.claude-plugin/marketplace.json` | superdev 条目 version、description 必须与 plugin.json 一致 |
| `superdev/README.md` | **Version:**、**Registered skills (N)** 及分组、v1.2.x |
| `README.md` + `README.zh-CN.md`（顶层） | superdev 段的 v1.2.x（**手动同步**，`/utils:update-readme` 禁模型，见记忆 project_readme_sync_manual） |
