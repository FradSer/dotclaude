#!/usr/bin/env bash
#
# Vercel Agent Skills 同步脚本
# 从 vercel-labs/agent-skills 仓库同步 react-best-practices 和 web-design-guidelines
#
...
SNAPSHOT_KEY="vercel-skills"

# 要同步的 skill 目录
SKILL_DIRS=("react-best-practices" "web-design-guidelines")
...
show_help() {
    cat << EOF
${BLUE}Vercel Agent Skills 同步脚本${NC}
...
clone_upstream() {
    ...
    for skill in "${SKILL_DIRS[@]}"; do
        for excl in "${EXCLUDE_DIRS[@]}"; do
            if [ -d "$TEMP_DIR/repo/skills/$skill/$excl" ]; then
                rm -rf "$TEMP_DIR/repo/skills/$skill/$excl"
                log_info "已排除 $skill/$excl/"
            fi
        done
    done
...
main() {
    ...
    log_info "开始同步 Vercel agent skills..."
    check_requirements
    clone_upstream
