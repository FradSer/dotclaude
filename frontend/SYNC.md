...
- **本地非同步补充文件（sync wipe 时保留,2026-06-20 P1-b Option B；2026-06-21 改 `.local.md` 后缀避免 upstream 同名覆盖）**: `skills/impeccable/PLUGIN-INSTALL-NOTES.local.md`（插件布局脚本路径 caveat + 解析配方）、`skills/impeccable/AUDIT-AUTHORITY.local.md`（四权威 evidence-type 对账阶梯 + Reachability gap 声明）。二者用 `.local.md` 后缀命名，upstream 文件无此后缀，从结构上消除同名覆盖；sync-impeccable.sh wipe 阶段对 `*.local.md` 一律 skip。`hooks/design-md-first.sh`（UserPromptSubmit hook,cwd 有 DESIGN.md 时注入 design-md-first + token authority ladder preamble）。
